...
-- 1. Update existing events RLS policy to be more restrictive
DROP POLICY IF EXISTS "Public can view active events for registration" ON public.events;

-- Create restricted public access policy for events (only essential registration data)
...
-- 2. Create rate limiting for participant registrations
...
-- 3. Create function to check and enforce rate limits
...
-- 4. Create validation function for participant data
...
-- 5. Create trigger to validate participant registrations
...
-- 6. Update participants table RLS policy to be more restrictive
